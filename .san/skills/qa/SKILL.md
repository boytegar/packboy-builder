...
name: qa
description: >-
  Regression test a Packboy Builder feature by name. Looks for a feature doc in docs/reference/ or
...
Run automated and interactive regression tests for a Packboy Builder feature by name.
...
- The binary path is `./bin/pcb` (built by `make build`), not the installed `pcb`.
...
- For interactive tests, use `Ctrl+C` (`C-c`) to exit `pcb`, not `q` (which gets interpreted as user input).
