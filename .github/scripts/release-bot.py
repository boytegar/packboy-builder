#!/usr/bin/env python3
"""
Release Bot — Automates release PR creation for genai-io/pcb.
...
On each run:
  ...
  5. Rewrites CHANGELOG.md and cmd/pcb/main.go
...
def read_version_from_main_go():
    """Read the current version string from cmd/pcb/main.go."""
    m = re.search(r'var version = "(.*?)"', Path("cmd/pcb/main.go").read_text())
...
def main():
    ...
    if not latest_tag:
        ...
        if not fallback:
            print("::error:: No tag and no version in cmd/pcb/main.go — cannot proceed.")
            sys.exit(1)
    ...
    # 9. Update cmd/pcb/main.go
    main_go = Path("cmd/pcb/main.go")
    ...
    print(f"Updated cmd/pcb/main.go -> version {next_version}")

    # 10. Commit, push, create PR
    run(["git", "config", "user.name", "pcb-release-bot[bot]"])
    run(["git", "config", "user.email", "pcb-release-bot@genai-io.users.github.com"])
    run(["git", "checkout", "-b", branch])
    run(["git", "add", "CHANGELOG.md", "cmd/pcb/main.go"])
