...
func TestLoadAgentsProjectOverridesClaudeCompat(t *testing.T) {
	...
	cwd := t.TempDir()
	writeAgentFile(t, filepath.Join(cwd, ".pcb", "agents"), "reviewer",
		"name: reviewer\ndescription: pcb project reviewer")
	...
	if config.Description != "pcb project reviewer" {
		t.Fatalf("description = %q, want the .pcb definition to win", config.Description)
	}
}
