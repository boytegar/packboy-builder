...
// auto-memory store: ~/.pcb/projects/<encoded-cwd>/memory/. It shares the
...
// agent-written memory and user-authored AGENTS.md/CLAUDE.md instructions are
...
func LoadMemoryFiles(cwd string) []MemoryFile {
	...
	// User-authored instruction files only: AGENTS.md and CLAUDE.md.
	// SAN.md is intentionally not loaded. Every matching file is loaded
	// (not first-wins) so a project can ship both AGENTS.md and CLAUDE.md.
	userSources := []string{
		filepath.Join(userDir, "AGENTS.md"),
		filepath.Join(homeDir, "AGENTS.md"),
		filepath.Join(homeDir, ".claude", "CLAUDE.md"),
		filepath.Join(homeDir, "CLAUDE.md"),
	}
	files = append(files, loadAllMemoryFiles(userSources, "global", seen)...)
	...
	files = append(files, loadAllMemoryFiles(projectSources, "project", seen)...)
...
// loadAllMemoryFiles loads every existing source at the given level (order
// preserved). Unlike loadMemoryFile, it does not stop at the first hit — a
// project that ships both AGENTS.md and CLAUDE.md contributes both.
func loadAllMemoryFiles(sources []string, level string, seen map[string]bool) []MemoryFile {
	var out []MemoryFile
	for _, src := range sources {
		if f := loadMemoryFile([]string{src}, level, seen); f != nil {
			out = append(out, *f)
		}
	}
	return out
...
// Only AGENTS.md and CLAUDE.md are instruction sources; SAN.md is not used.
func GetAllMemoryPaths(cwd string) MemoryPaths {
	homeDir, _ := os.UserHomeDir()
	return MemoryPaths{
		Global: []string{
			filepath.Join(confdir.Dir(homeDir), "AGENTS.md"),
			filepath.Join(homeDir, "AGENTS.md"),
			filepath.Join(homeDir, ".claude", "CLAUDE.md"),
			filepath.Join(homeDir, "CLAUDE.md"),
		},
