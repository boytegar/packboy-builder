// Package skill provides skill management for Packboy Builder.
...
const (
	// ScopeBuiltin is skills embedded in the Packboy Builder binary (lowest priority).
	// Materialized to a cache dir so references are readable via the Read tool.
	ScopeBuiltin SkillScope = iota

	// ScopeClaudeUser is ~/.claude/skills/ (Claude compatibility)
	ScopeClaudeUser

	// ScopeUserPlugin is ~/.pcb/plugins/*/skills/ (User plugins)
	ScopeUserPlugin
	...
	// ScopeProjectPlugin is .pcb/plugins/*/skills/ (Project plugins)
	ScopeProjectPlugin

	// ScopeProject is .pcb/skills/ (Packboy Builder project level, highest priority)
	ScopeProject
