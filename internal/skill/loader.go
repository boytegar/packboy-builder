...
func (l *loader) getSearchPaths() []searchPath {
	...
	// 0. Embedded builtin skills (lowest priority; user/project override)
	if root, err := materializeBuiltinSkills(); err == nil && root != "" {
	...
	// 1. ~/.claude/skills/ (Claude user compat)
	paths = append(paths, searchPath{
