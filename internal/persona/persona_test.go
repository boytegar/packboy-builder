...
func TestRegistry_ProjectOverridesUser(t *testing.T) {
	...
	writeFile(t, filepath.Join(home, ".pcb", "personas", "ml", "settings.json"),
		`{"description":"user-level ml"}`)
	writeFile(t, filepath.Join(cwd, ".pcb", "personas", "ml", "settings.json"),
...
func TestRegistry_ListDefaultFirst(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeFile(t, filepath.Join(home, ".pcb", "personas", "zeta", "settings.json"), `{"description":"z"}`)
...
func TestRegistry_LoadDirSkipsDefaultName(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeFile(t, filepath.Join(home, ".pcb", "personas", "default", "settings.json"), `{"description":"reserved"}`)
...
func TestIsPersonaFile(t *testing.T) {
	...
	inside := []string{
		filepath.Join(home, ".pcb", "personas", "ml", "system", "identity.md"),
		filepath.Join(cwd, ".pcb", "personas", "go", "settings.json"),
		filepath.Join(cwd, ".pcb", "personas", "go", "skills", "x", "SKILL.md"),
	}
	...
	outside := []string{
		filepath.Join(home, ".pcb", "identities", "ml.md"),    // identities are no longer loaded
		filepath.Join(cwd, "personas", "go", "settings.json"), // not under .pcb
		"",
	}
...
func TestRegistry_Validate(t *testing.T) {
	...
	writeFile(t, filepath.Join(home, ".pcb", "personas", "ml", "settings.json"), `{"description":"ml"}`)
...
func TestEnsureUserDir_IsIdempotent(t *testing.T) {
	...
	readme := filepath.Join(home, ".pcb", "personas", "README.md")
