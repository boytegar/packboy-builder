...
func resolveProvider(ctx context.Context) (llm.Provider, string, error) {
	...
	if !ok {
		return nil, "", fmt.Errorf("no provider connected. Run 'pcb' and use /models to connect")
	}
	...
	if modelID == "" {
		...
		return nil, "", fmt.Errorf("no model selected for %s. Run 'pcb' and use /models to select one", resolved.Provider.Name())
	}
