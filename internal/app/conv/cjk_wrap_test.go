...
func TestBlockquoteCJKKeepsPrefixAndWidth(t *testing.T) {
	src := "> 最近 Packboy Builder 的几次更新，表面上看是 Persona、Self-Learning、Subscription、Autopilot。但放在一起看，它们其实都指向同一个问题：一个 Agent Harness，应该怎样帮助开发者把 Agent 真正带进工作流？"
...
func TestBlockquoteCJKPreservesContent(t *testing.T) {
	src := "> 最近 Packboy Builder 的几次更新，指向同一个问题：一个 Agent Harness 应该怎样帮助开发者把 Agent 真正带进工作流？"
