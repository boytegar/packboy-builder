...
// Run:  PCB_BASH_RENDER_DEMO=1 go test ./internal/app/conv/ -run TestBashRenderDemo -v
// Tweak the `width` and add your own cases to `samples`.
func TestBashRenderDemo(t *testing.T) {
	if os.Getenv("PCB_BASH_RENDER_DEMO") == "" {
		t.Skip("set PCB_BASH_RENDER_DEMO=1 to print the Bash rendering demo")
	}
