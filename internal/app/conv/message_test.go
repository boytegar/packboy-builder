...
func Test_extractToolArgsPreservesFullCommand(t *testing.T) {
	input := `{"command":"cd /Users/myan/Workspace/ideas/pcb && git describe --tags --abbrev=0 2>/dev/null"}`
...
func Test_renderBashToolCallMovesOversizedSingleLineCommandToFullBlock(t *testing.T) {
	command := "gh pr view 414 --repo genai-io/pcb --json number,title,state,url,author,baseRefName,headRefName,headRepositoryOwner,body,comments,reviews,reviewRequests,statusCheckRollup"
...
func TestRenderToolCallsWrapsLongSingleLineBashCommandWithoutTruncating(t *testing.T) {
	...
	params := ToolCallsParams{
		ToolCalls: []core.ToolCall{{
			ID:    "tc-1",
			Name:  "Bash",
			Input: `{"command":"cd /Users/myan/Workspace/ideas/pcb && git describe --tags --abbrev=0 2>/dev/null","description":"inspect the latest repository tag and ignore command errors"}`,
		}},
	...
	fullCommand := "Bash(cd /Users/myan/Workspace/ideas/pcb && git describe --tags --abbrev=0 2>/dev/null)"
