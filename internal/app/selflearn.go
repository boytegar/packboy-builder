...
import (
	...
	"github.com/boytegar/packboy-builder/internal/selflearn"
...
func (m *model) wireSelfLearn(params agent.BuildParams) {
	...
	review := func(kinds selflearn.ReviewKind, skillPerms selflearn.SkillPermissions, snapshot []core.Message) {
		...
		// `pcb --resume <fork-id>` replays exactly that review's LLM
...
// suggest "pcb --resume <id>" for replay.
...
// card with the "pcb --resume" hint as a separate line below — no more
...
//	╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄╯
//	  ↪ pcb --resume demo-session.selflearn-review.123
//
...
func formatRecapBlock(actions []ReviewAction, sessionID string) string {
	...
	if sessionID != "" {
		...
		out += "\n  " + selflearnRecapFooterStyle.Render("↪ pcb --resume "+sessionID)
	}
	return out
}
...
var (
	...
	selflearnRecapBoxStyle = lipgloss.NewStyle().
		...
		Padding(0, 2)
	// Footer style for "↪ pcb --resume <id>" on its own line below the
