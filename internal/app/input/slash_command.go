...
func builtinCommandHandlers() map[string]slashCommandHandler {
	return map[string]slashCommandHandler{
		...
		"yolo":           (*SlashCommandController).handleYoloCommand,
...
// clears the override back to Packboy Builder's built-ins).
...
// YoloMsg asks the app to set or toggle bypass-permissions mode (auto-accept
// all tool calls after deny rules and the root/home circuit breaker).
// Enable is nil for toggle, non-nil for an explicit on/off.
type YoloMsg struct {
	Enable *bool
}

// handleYoloCommand toggles bypass permissions. Bare /yolo flips the mode;
// /yolo on and /yolo off set it explicitly. Rejected when allowBypass is false.
func (c *SlashCommandController) handleYoloCommand(_ context.Context, args string) (string, tea.Cmd, error) {
	...
	case "off", "disable", "false", "0":
		...
		enable = &v
	default:
		return "Usage: /yolo [on|off]\n\nToggle bypass permissions (auto-accept all tool calls).", nil, nil
	}
	// Early gate for explicit enable so the user gets feedback without a mode flip.
	if enable != nil && *enable && c.env.Setting != nil && !c.env.Setting.AllowBypass() {
		return "Bypass permissions is locked out (allowBypass: false in settings).", nil, nil
	}
	return "", func() tea.Msg { return YoloMsg{Enable: enable} }, nil
}
