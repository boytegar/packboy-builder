...
func TestAppearancePanelEnterSavesAndEmits(t *testing.T) {
	...
	raw, err := os.ReadFile(filepath.Join(home, ".pcb", "settings.json"))
	if err != nil {
...
func TestAppearancePanelEnterSaveFailureSurfacesError(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	// Block the write: a regular file where the .pcb dir must be makes the
	// loader's MkdirAll fail.
	if err := os.WriteFile(filepath.Join(home, ".pcb"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
