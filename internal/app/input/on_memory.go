...
func (m *MemorySelector) EnterSelect(cwd string, width, height int) {
	...
	m.items = []memoryItem{
		...
		m.buildMemoryItem("Project", "project", paths.Project, cwd,
			"Checked in at AGENTS.md / CLAUDE.md",
			"Use /init to create AGENTS.md"),
	}
}
...
func handleInitProject(cwd string, isClaude bool) (string, error) {
	...
	if isClaude {
		...
		// Project root AGENTS.md — the primary runtime instruction file.
		targetDir = cwd
		fileName = "AGENTS.md"
	}
...
func handleInitLocal(cwd string) (string, error) {
	// Local-only memory files are no longer part of the load path.
	// Keep the subcommand so existing muscle memory fails clearly.
	_ = cwd
	return "Local memory (SAN.local.md) is no longer loaded.\nUse /init to create AGENTS.md, or /init --claude for CLAUDE.md.", nil
}
...
func handleMemoryEdit(cwd, scope string) (string, error) {
	...
	case "local":
		return "", fmt.Errorf("local memory is no longer loaded; use /memory edit project (AGENTS.md) or /init --claude")
...
This file provides guidance to Packboy Builder when working with code in this repository.
...
Global instructions for Packboy Builder (applies to all projects).
...
Local-only notes are no longer auto-loaded. Prefer AGENTS.md or CLAUDE.md.
`
}
...
This file defines specific rules for Packboy Builder to follow.
