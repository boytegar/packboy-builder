...
import (
	...
	"github.com/boytegar/packboy-builder/internal/reviewer"
...
func (m *model) buildAgentParams() agent.BuildParams {
	...
	// Stream timeout overrides. Priority: PCB_* env var > settings.json > core
...
func (m *model) preparePermissionRequest(req *conv.PermGateRequest) *perm.PermissionRequest {
	var rich *perm.PermissionRequest
	if resolved, ok := tool.Get(req.ToolName); ok {
		if pat, ok := resolved.(tool.PermissionAwareTool); ok {
			if r, err := pat.PreparePermission(context.Background(), req.Input, m.env.CWD); err == nil && r != nil {
				rich = r
			}
		}
	}
	if rich == nil {
		rich = &perm.PermissionRequest{
			ToolName:    req.ToolName,
			Description: req.Description,
		}
	}
	// Always-allow needs concrete patterns. Tools rarely fill SuggestedRules
	// themselves; generate them here so the modal label and Persist path share
	// the same rules.
	if len(rich.SuggestedRules) == 0 {
		rich.SuggestedRules = setting.GenerateSuggestions(req.ToolName, req.Input, setting.MaxSuggestedRules)
	}
	if len(rich.SuggestedRules) == 0 {
		if rule := setting.BuildRule(req.ToolName, req.Input); rule != "" {
			rich.SuggestedRules = []string{rule}
		}
	}
	return rich
}
