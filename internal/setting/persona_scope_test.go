...
func TestPersonaAt_ReadsEachScopeRaw(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(home, ".pcb"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(home, ".pcb", "settings.json"), []byte(`{"persona":"user-p"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(cwd, ".pcb"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(cwd, ".pcb", "settings.json"), []byte(`{"persona":"proj-p"}`), 0o644); err != nil {
		t.Fatal(err)
	}
