...
// NewLoader creates a loader with default paths (~/.pcb, .pcb) and Claude compatibility enabled.
...
//  6. .pcb/settings.local.json
func (l *Loader) Load() (*Data, error) {
	homeDir, _ := os.UserHomeDir()

	// Two-phase loading: first load Claude-compat settings, then Packboy Builder-native.
	// For hooks, Packboy Builder-native settings override Claude-compat settings per event
	// to prevent incompatible hooks (e.g., Claude Code's interactive protocol)
	// from blocking Packboy Builder's own hooks.
	...
	// Collect hooks separately: Claude-compat hooks and Packboy Builder-native hooks.
	...
	for _, src := range sources {
		...
		// This means project .pcb/settings.json can set "PermissionRequest": []
	...
	// incompatible with Packboy Builder's TUI-based approval flow and can cause hangs.
...
// userLevel=true reads from ~/.pcb/settings.json; false reads from .pcb/settings.json.
...
// userLevel=true reads ~/.pcb/settings.json; false reads <cwd>/.pcb/settings.json.
...
// SaveTheme persists the chosen theme to ~/.pcb/settings.json.
...
// ~/.pcb/settings.json. The value is written as an explicit pointer so an
