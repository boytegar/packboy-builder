...
func TestConfig_LocalOverridesProject(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(tmpProject, ".pcb"), 0o755); err != nil {
		t.Fatalf("Failed to create project .pcb dir: %v", err)
	}
	if err := os.WriteFile(
		filepath.Join(tmpProject, ".pcb", "settings.json"),
	...
	if err := os.WriteFile(
		filepath.Join(tmpProject, ".pcb", "settings.local.json"),
		[]byte(localSettings), 0o644,
	); err != nil {
		t.Fatalf("Failed to write settings.local.json: %v", err)
	}

	loader := NewLoaderWithOptions(tmpUser, filepath.Join(tmpProject, ".pcb"), false)
...
func TestConfig_LocalOverridesProject_MergesNotReplaces(t *testing.T) {
	tmpUser := t.TempDir()
	tmpProject := t.TempDir()
	sanDir := filepath.Join(tmpProject, ".pcb")
	if err := os.MkdirAll(sanDir, 0o755); err != nil {
		t.Fatalf("Failed to create .pcb dir: %v", err)
	}
