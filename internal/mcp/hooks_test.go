...
func TestConfigSourceFromFilePath(t *testing.T) {
	if got := configSourceFromFilePath("/repo/.pcb/mcp.local.json"); got != "local_settings" {
		t.Fatalf("unexpected local source: %q", got)
	}
	if got := configSourceFromFilePath("/repo/.pcb/mcp.json"); got != "project_settings" {
	...
	userPath := filepath.Join(homeDir, ".pcb", "mcp.json")
