...
//  1. ~/.pcb/mcp.json (user scope)
//  2. ./.pcb/mcp.json (project scope)
//  3. ./.pcb/mcp.local.json (local scope)
...
func configSourceFromFilePath(filePath string) string {
	...
	case "mcp.json":
		// A user-level mcp.json lives at ~/.pcb/mcp.json. Check it before the
		// project-settings fallthrough, since a project path <cwd>/.pcb/mcp.json
		// also contains "/.pcb/".
