...
func TestExpandPluginRoot(t *testing.T) {
	tests := []struct {
		...
		{
			input:      "${PCB_PLUGIN_ROOT}/scripts/test.sh",
...
func TestLoadPlugin(t *testing.T) {
	...
	// Create .pcb-plugin/plugin.json
	pluginMetaDir := filepath.Join(tmpDir, ".pcb-plugin")
...
func TestRegistry_EnableDisable_PersistsScopedSettings(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			scope:        ScopeUser,
			enable:       true,
			settingsPath: filepath.Join(tmpHome, ".pcb", "settings.json"),
		},
		{
			name:         "project disable",
			scope:        ScopeProject,
			enable:       false,
			settingsPath: filepath.Join(tmpCwd, ".pcb", "settings.json"),
		},
		{
			name:         "local disable",
			scope:        ScopeLocal,
			enable:       false,
			settingsPath: filepath.Join(tmpCwd, ".pcb", "settings.local.json"),
		},
	}
...
func writeTestPlugin(t *testing.T, root, name, version, description string, extraFiles map[string]string) {
	...
	metaDir := filepath.Join(root, ".pcb-plugin")
...
func TestInstaller_InstallAndUninstall_FromMarketplaceDirectory(t *testing.T) {
	...
	installPath := filepath.Join(cwd, ".pcb", "plugins", "deploy")
...
func TestRegistry_LoadScopeMergePrefersLocalOverProjectOverUser(t *testing.T) {
	...
	writeTestPlugin(t, filepath.Join(tmpHome, ".pcb", "plugins", "shared"), "shared", "1.0.0", "user plugin", nil)
	writeTestPlugin(t, filepath.Join(cwd, ".pcb", "plugins", "shared"), "shared", "1.0.0", "project plugin", nil)
	writeTestPlugin(t, filepath.Join(cwd, ".pcb", "plugins-local", "shared"), "shared", "1.0.0", "local plugin", nil)
...
func TestPlugin_LSPLoading(t *testing.T) {
  ...
  "go": {
	...
	manifestPath := filepath.Join(tmpDir, ".pcb-plugin", "plugin.json")
...
func TestValidatePlugin(t *testing.T) {
	...
	noNameDir := t.TempDir()
	noNameMetaDir := filepath.Join(noNameDir, ".pcb-plugin")
...
func TestPlugin_Validate_InvalidManifest(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "missing_name",
			manifestFn: func(dir string) error {
				metaDir := filepath.Join(dir, ".pcb-plugin")
			...
			"PostToolUse": [
				{
					...
					"hooks": [
						{
							"type": "command",
							"command": "${PCB_PLUGIN_ROOT}/scripts/format.sh",
							"async": true
						}
					]
				}
...
func TestMCPConfigParsing(t *testing.T) {
	...
	mcpJSON := `{
		"mcpServers": {
			"database": {
				"command": "${PCB_PLUGIN_ROOT}/servers/db",
				"args": ["--config", "${PCB_PLUGIN_ROOT}/config.json"],
				"env": {
					"DB_PATH": "${PCB_PLUGIN_ROOT}/data"
				}
			}
		}
	}`
