...
const (
	// SanPluginDir is the directory containing plugin metadata for Packboy Builder
	SanPluginDir = ".pcb-plugin"
...
// It looks for .pcb-plugin/plugin.json or .claude-plugin/plugin.json.
...
// loadManifest loads the plugin manifest from .pcb-plugin or .claude-plugin
...
func LoadPluginsFromDir(dir string, scope Scope, sourcePrefix string) ([]*Plugin, error) {
	...
	for _, entry := range entries {
		...
		// Skip hidden directories (except .claude* and .pcb*)
		if strings.HasPrefix(name, ".") && !strings.HasPrefix(name, ".claude") && !strings.HasPrefix(name, ".pcb") {
			continue
		}
