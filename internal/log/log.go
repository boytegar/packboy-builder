...
// debugEnabled reports whether debug logging is on via PCB_DEBUG=1. Kept local
// because the log package is an infrastructure leaf and cannot import
// internal/setting.
func debugEnabled() bool {
	return os.Getenv("PCB_DEBUG") == "1"
}

// Init initializes the logger based on PCB_DEBUG env var
func Init() error {
	mu.Lock()
	defer mu.Unlock()
	...
	// Initialize DEV_DIR for JSON debug output (independent of PCB_DEBUG)
