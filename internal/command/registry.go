...
import (
	...
	"github.com/boytegar/packboy-builder/internal/markdown"
...
func builtinCommands() []Info {
	return []Info{
		...
		{Name: "think", Description: "Set or cycle thinking effort (none/low/medium/high/xhigh)"},
		...
		{Name: "refine", Description: "Review trajectory and apply evidence-backed updates to harness state (prompts, memories, skills, subagent specs)"},
		...
		{Name: "yolo", Description: "Toggle bypass permissions — auto-accept all tool calls (/yolo, /yolo on|off)"},
...
const (
	scopeUser          commandScope = iota // ~/.pcb/commands/
	scopeUserPlugin                        // ~/.pcb/plugins/*/commands/
	scopeProjectPlugin                     // .pcb/plugins/*/commands/
	scopeProject                           // .pcb/commands/
	scopeBuiltin                           // embedded in the binary (internal/command/prompts/)
)

// CustomCommand represents a user-defined slash command from
// ~/.pcb/commands/, .pcb/commands/, or a plugin's commands/ directory.
...
// 1. ~/.pcb/commands/        (user level, lowest priority)
// 2. ~/.pcb/plugins/*/commands/ (user-plugin)
// 3. .pcb/plugins/*/commands/   (project-plugin)
// 4. .pcb/commands/          (project level, highest priority)
