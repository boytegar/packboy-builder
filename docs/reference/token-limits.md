...
Token limits help track context window usage and prevent exceeding model limits. Packboy Builder supports both provider-supplied limits (from API) and user-configured limits.

## Storage

Token limits are stored in `~/.pcb/providers.json`:
