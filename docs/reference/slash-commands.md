...
TestHandleInitCommand                     — /init creates AGENTS.md at project root
TestHandleInitCommand (local)             — /init local explains local is no longer loaded
TestHandleInitCommand (rules)             — /init rules creates .pcb/rules directory
...
tmux send-keys -t t_cmds 'pcb' Enter
sleep 2
...
tmux send-keys -t t_cmds 'mkdir -p /tmp/init_test && cd /tmp/init_test && pcb' Enter
sleep 2
tmux send-keys -t t_cmds '/init' Enter
sleep 3
ls /tmp/init_test/AGENTS.md
# Expected: AGENTS.md created at project root
