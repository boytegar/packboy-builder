...
| 2 | `~/.pcb/settings.json` (Packboy Builder user) |
| 3 | `./.claude/settings.json` (Claude project compat) |
| 4 | `./.pcb/settings.json` (Packboy Builder project) |
...
```bash
mkdir -p /tmp/cfg_test/.pcb

# User-level env var (backup existing)
cp ~/.pcb/settings.json ~/.pcb/settings.json.bak 2>/dev/null || true
cat > ~/.pcb/settings.json << 'EOF'
{"env": {"SCOPE": "user"}}
EOF

# Project-level overrides it + disables a tool
cat > /tmp/cfg_test/.pcb/settings.json << 'EOF'
{"env": {"SCOPE": "project"}, "disabledTools": {"WebSearch": true}}
EOF

tmux new-session -d -s t_cfg -x 220 -y 60
tmux send-keys -t t_cfg 'cd /tmp/cfg_test && pcb' Enter
sleep 2
...
cat > /tmp/cfg_test/.pcb/settings.local.json << 'EOF'
{"env": {"SCOPE": "local"}}
EOF
tmux send-keys -t t_cfg 'cd /tmp/cfg_test && pcb' Enter
sleep 2
...
mv ~/.pcb/settings.json.bak ~/.pcb/settings.json 2>/dev/null || true
