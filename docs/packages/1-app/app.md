---
package: github.com/boytegar/packboy-builder/internal/app
layer: app
---
...
- **`services` snapshots 16 singletons via `Default()` at construction.**
  ...
  is for `cmd/pcb` to construct each service explicitly and pass it in,
...
- `cmd/pcb` calls `app.Run()` which builds the `tea.Program`,
