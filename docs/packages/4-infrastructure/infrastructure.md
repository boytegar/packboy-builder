...
- `log.Init()` runs once at app startup (from `internal/app/init.go`).
  Output is suppressed by default; `PCB_DEBUG=1` enables zap with
  `lumberjack` rotation.
...
Filesystem-backed key/value store under `~/.pcb/secrets.json` for API
keys and tokens.
...
- Consumers: [`skill.md`](../2-feature/skill.md), [`subagent.md`](../2-feature/subagent.md),
  ...
  persona / command file is parsed through it on `pcb` startup.
