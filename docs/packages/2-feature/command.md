---
package: github.com/boytegar/packboy-builder/internal/command
layer: feature
---
...
- `registry.go` — combines three sources at query time:
  ...
  - **Custom** — markdown files under `~/.pcb/commands/` and
    `<project>/.pcb/commands/`, plus plugin-scoped paths returned by
    `PluginCommandPaths`.
