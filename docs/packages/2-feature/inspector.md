---
package: github.com/boytegar/packboy-builder/internal/inspector
layer: feature
---
...
under `~/.pcb/projects/<cwd-hash>/<session-id>.jsonl` and exposes a
small HTTP API + SSE live-tail to an embedded single-page UI.

## Purpose

When the user runs `pcb inspector`, this package serves a localhost-only
