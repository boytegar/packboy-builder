---
package: github.com/boytegar/packboy-builder/internal/session
layer: feature
---
...
Each pcb run is a session. Sessions are auto-saved as JSONL transcripts
under `~/.pcb/projects/<workdir-hash>/`. `--continue` and `--resume` use the
...
- `Store` (`store.go`) — filesystem-backed JSON store under
  `~/.pcb/projects/<hash>/`; provides Save / Load / List / Fork.
