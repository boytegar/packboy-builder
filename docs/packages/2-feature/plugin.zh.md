...
Packboy Builder 的"其余一切"扩展面。插件让用户一次性安装一整套 skills + agents +
...
- `Registry`（`registry.go`）——以 name 为键的 `Plugin` map，按 scope
  （user / project）保存启用状态。
- `loader.go`——发现 `.pcb/plugins/`、`~/.pcb/plugins/`、
...
**scope → 安装目录：** user → `~/.pcb/plugins/cache/`，project →
`.pcb/plugins/`，local → `.pcb/plugins-local/`。启用状态保存在对应 scope
