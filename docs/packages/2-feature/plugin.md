---
package: github.com/boytegar/packboy-builder/internal/plugin
layer: feature
---
...
Packboy Builder's "everything else" extension surface. Plugins are how a user
...
- `loader.go` — discovers `.pcb/plugins/`, `~/.pcb/plugins/`,
...
flowchart TD
    ...
    B --> C["Sync: git clone --depth 1<br/>→ ~/.pcb/plugins/marketplaces/&lt;id&gt;"]
...
**Scope → install dir:** user → `~/.pcb/plugins/cache/`, project →
`.pcb/plugins/`, local → `.pcb/plugins-local/`. Enable state lives in
