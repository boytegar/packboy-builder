---
package: github.com/boytegar/packboy-builder/internal/llm
layer: feature
---
...
- `Store` (`store.go`) — persists user's provider connections under
  `~/.pcb/providers.json`; tracks current model and caches provider-scoped
...
- Construction: `Initialize(Options{})` loads `~/.pcb/providers.json`,
