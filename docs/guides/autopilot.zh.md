...
Autopilot 是 Packboy Builder 的自动驾驶系统,旨在最大限度减少人工介入:由一个 copilot
...
用 `/autopilot` 面板配置。恢复会话(`pcb -r <id>`)会回到保存时所在的模式。
...
**1. 在一个空仓库里启动 Packboy Builder。** 只在那里跑 —— 下面这条 goal 会在启动目录下直接
...
菜单里,`e` 导出当前配置、`i` 导入,存取于 `~/.pcb/autopilot/<name>.json`。
