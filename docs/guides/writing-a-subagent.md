...
| `max-steps` | no | Maximum LLM steps; 0 or unset = unlimited. Alias: `max_steps`. |
...
- **Same name in several locations.** Priority is project `.pcb` > user
  `~/.pcb` > project `.claude` > user `~/.claude` > plugins; the
