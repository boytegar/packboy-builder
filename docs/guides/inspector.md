...
The **Inspector** is a local web UI for exploring and debugging pcb code
session transcripts. It reads the JSONL transcript files stored under
`~/.pcb/projects/` and presents every event — user messages, model
...
```bash
# Pin a specific port
pcb inspector --addr 127.0.0.1:38080

# Print the URL but don't open the browser
pcb inspector --no-open
```
...
When you have an active pcb code session running in another terminal, the
...
1. `pcb inspector` starts an HTTP server bound to loopback.
2. The server reads transcript JSONL files from
   `~/.pcb/projects/<encoded-cwd>/transcripts/`.
