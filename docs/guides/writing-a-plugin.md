...
| Scope | Path |
|---|---|
| User | `~/.pcb/plugins/<name>/` |
| Project | `<project>/.pcb/plugins/<name>/` |
...
pcb plugin install ./my-plugin

# From a git URL
pcb plugin install https://github.com/you/github-flow

# From the marketplace (if configured)
pcb plugin install github-flow
```

`pcb plugin install` runs the installer in `internal/plugin/installer.go`,
...
State is persisted in `~/.pcb/plugins.json` and
`<project>/.pcb/plugins.json`.
...
Configure marketplace URLs in `~/.pcb/settings.json`:
...
`pcb plugin search` and `pcb plugin install <name>` then resolve through
the marketplace.

## Common Pitfalls

- **Forgot `pluginspec.json`.** Plugin is silently skipped at load time.
  Check `~/.pcb/logs/` for the warning.
