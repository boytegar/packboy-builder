...
Providers connect Packboy Builder to an LLM vendor. Most providers live under
...
```go
_ "github.com/boytegar/packboy-builder/internal/llm/<provider>"
```
...
- **Skipping registration.** If the package is not imported from
  `cmd/pcb/main.go`, `/models` will not discover it.
