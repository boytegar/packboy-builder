...
surface in Packboy Builder — one file, no install step.
...
`./.pcb/skills/release-notes/SKILL.md`:
...
Resources are lazy-loaded — Packboy Builder does not read them until the skill
is actually invoked.
...
`~/.pcb/skills.json` (user level) or `<project>/.pcb/skills.json`
(project level).

## Trying It

1. Save the SKILL.md as above.
2. Run `pcb` (or restart your session — skills are discovered at startup).
...
- Commit `./.pcb/skills/<name>/` into the project repo — collaborators
  get it on next `pcb` run.
- For cross-project skills, put them under `~/.pcb/skills/` and back
...
- **Project state shadows user state.** Toggling a skill at user level
  has no effect if a `<project>/.pcb/skills.json` entry exists for the
  same name.
