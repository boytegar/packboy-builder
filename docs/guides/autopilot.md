...
Autopilot is Packboy Builder's autonomy system, designed to minimize human intervention: a
...
session (`pcb -r <id>`) comes back in the mode it was saved in. If you just want
...
**1. Start Packboy Builder in an empty repository.** Run it there and nowhere else — the
goal below writes `notes/` wherever it starts, so in one of your own projects it
would scribble into a real directory:

```bash
mkdir /tmp/autopilot-demo && cd /tmp/autopilot-demo && git init -q && pcb
```
...
one, stored under `~/.pcb/autopilot/<name>.json`.
