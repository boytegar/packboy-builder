...
Packboy Builder ships as **one self-contained binary**. There is no Node.js runtime,
...
| Dimension | Packboy Builder | Why it matters |
|---|---|---|
...
heavier baseline in RAM and startup time. Packboy Builder carries its entire world
inside the single executable.

## Why a single static binary

Packboy Builder is compiled with Go. On Linux the released binaries are **statically
...
curl -fsSL https://raw.githubusercontent.com/genai-io/pcb/main/install.sh | bash
pcb --version
```

The installer fetches the matching `linux/arm64` archive (~12 MB), unpacks the
single `pcb` binary, and puts it on your `PATH`. Nothing else is installed.
...
GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=0 \
  go build -ldflags "-s -w" -o pcb ./cmd/pcb
```

## Summary

Small is not just a vanity metric. A ~12 MB, zero-dependency, single-file binary
is what lets Packboy Builder run on a Raspberry Pi, slip into a `scratch` container,
