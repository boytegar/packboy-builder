# Packboy Builder vs Claude Code: Performance Benchmark

Comparison between **Packboy Builder v1.13.2** (Go) and **Claude Code v2.1.112** (Node.js/TypeScript).
...
| Metric | Packboy Builder | Claude Code | Ratio |
|--------|---------|-------------|-------|
...
Packboy Builder ships as a single static binary with zero runtime dependencies. Claude Code requires Node.js and installs ~63 MB of npm packages.
...
| Run | Packboy Builder | Claude Code |
|-----|---------|-------------|
| 1 | 0.01s | 0.20s |
| 2 | 0.01s | 0.19s |
| 3 | 0.01s | 0.20s |
...
Packboy Builder starts **~20x faster**.
...
Packboy Builder uses **~5.8x less memory** at startup. The Node.js runtime alone accounts for a large portion of Claude Code's baseline.
...
| Run | Packboy Builder (time / RSS) | Claude Code (time / RSS) |
|-----|----------------------|--------------------------|
...
- Response time: Packboy Builder **~4.3x faster**
- Memory: Packboy Builder **~7.3x less**
...
- Response time: Packboy Builder **~6.2x faster**
- Memory: Packboy Builder **~7.3x less**
...
- Response time: Packboy Builder **~7.9x faster**
- Memory: Packboy Builder **~7.2x less**

---

## 7. Context Overhead (First Turn)

Measured separately from the runs above, on **Packboy Builder v1.22.0** and **Claude Code v2.1.220** (2026-07-26). This is the fixed cost the harness itself imposes on every conversation: the system prompt plus the tool schemas, sent before the user's first message.
...
| Component | Packboy Builder | Claude Code |
|-----------|-----|-------------|
...
Packboy Builder is **~9x leaner** in context before any work begins.
...
- The two sides use different counting paths: Packboy Builder's is a static count with `cl100k_base`, Claude Code's is the provider's own tokenizer on a live request. Claude's tokenizer typically yields modestly more tokens for the same text, so Packboy Builder's real figure is likely somewhat above 2.3k — the ratio is approximate, not exact.
- Packboy Builder's `Agent` schema embeds the available-agent directory at runtime, so a project with many subagent definitions adds to the static figure above.
...
Harness tokens are paid on every request that misses cache, and they permanently reduce the window left for the actual work. Packboy Builder keeps the fixed block small and cache-stable across turns, and loads memory, skills, and project instructions only when they are used.

---

## Summary

| Metric | Packboy Builder | Claude Code | Packboy Builder Advantage |
|--------|---------|-------------|-------------------|
...
- **Architecture**: Packboy Builder is a single static binary with zero dependencies. Claude Code is a bundled TypeScript application running on Node.js with npm dependencies.
