...
A **persona** is a single on-disk folder that bundles everything that makes Packboy Builder
...
Personas are Packboy Builder's single mechanism for "who Packboy Builder is": they unify the system
...
permissions) as a unit, reusing the hot-patch machinery Packboy Builder already has so the
...
voice ships a single `system/identity.md`; everything else uses Packboy Builder's defaults.
...
```
~/.pcb/personas/<name>/      ← user level
.pcb/personas/<name>/        ← project level (overrides user)
```
...
```
~/.claude → ~/.pcb → .claude → .pcb → *.local.json → [PERSONA] → env/CLI → managed
                                                          ▲
...
persona.Registry.Reload(cwd)
  ├─ scan ~/.pcb/personas/*/      (user)
  └─ scan <cwd>/.pcb/personas/*/  (project, overrides user)
...
|---|---|---|---|
| D1 | Directory & skills subdir | `~/.pcb/personas/` + `.pcb/personas/`, subdir `skills/` (no dot) | consistent with `.pcb/skills`; reuses the skill loader's `<scope>/skills/` scan |
| D2 | One "who Packboy Builder is" mechanism | persona, not a separate identity feature | one concept, one command |
