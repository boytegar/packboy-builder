...
When a conversation approaches the model's input limit, Packboy Builder **compacts**
...
The window itself is resolved as: `PCB_INPUT_LIMIT` if set, else the limit the
...
`PCB_INPUT_LIMIT` for a model Packboy Builder cannot size on its own.
