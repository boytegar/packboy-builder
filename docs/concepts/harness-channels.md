...
Packboy Builder delivers context to the model through **three distinct channels**,
...
Two memory tiers (`SAN.md` is not loaded):

- **User memory**: `~/.pcb/AGENTS.md`, `~/AGENTS.md`, `~/.claude/CLAUDE.md`,
  and `~/CLAUDE.md`. Every present file loads. Attached as `memory-user`.
- **Project memory**: `<project>/AGENTS.md`, `<project>/CLAUDE.md`, and
  `<project>/.claude/CLAUDE.md`. Every present file loads. Attached as
  `memory-project`.
