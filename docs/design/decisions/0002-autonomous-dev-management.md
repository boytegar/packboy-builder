...
Development of the Packboy Builder project (`genai-io/pcb`) currently relies on a
human to drive the agent in every session. Issues, features, and bugs
are not managed autonomously.

The goal is to make Packboy Builder a **fully AI-managed project**: humans only
...
`pcb-team` for storing the Packboy Builder development team's Personas.
...
├── pcb              ← Packboy Builder source repo (existing, the managed project)
└── pcb-team         ← New repo (Packboy Builder development team Personas)
    ├── leader/
    │   ├── system/
    │   │   ├── identity.md
    │   │   ├── behavior.md
...
- **`pcb-team`**: New repo containing the Packboy Builder project's development team.
  ...
  (e.g., `devops-team`).
- **Team**: A collection of Personas in the `pcb-team` repo that
  collaborate toward a specific goal — managing the Packboy Builder project's issues,
...
Each Persona runs as an **independent Packboy Builder instance**:

```bash
# Start Leader Persona (admin interaction entry point)
pcb start --persona leader --team pcb-team

# Start Dev Persona (waits for coding tasks)
pcb start --persona dev --team pcb-team

# Start QE Persona (waits for verification tasks)
pcb start --persona qe --team pcb-team

# Start Release Persona (waits for release tasks)
pcb start --persona release --team pcb-team
```

The `--persona` flag tells Packboy Builder to load a specific Persona directory's
...
work queue (`pcb-team/state/queue.jsonl`).
...
Admin (human)
    ...
    ▼
┌──────────────────────────────────────────────────────┐
│ Leader Persona (pcb start --persona leader --team pcb-team)│
│                                                      │
│ 1. Understand intent                                │
...
Started via `pcb start --persona leader --team pcb-team`.
The admin's only interface. Leader handles:

1. **Understand intent**: new feature? bug fix? refactor?
2. **Analyze Packboy Builder**: read design docs and existing code in the Packboy Builder repo
...
Persona's Packboy Builder instance picks them up automatically.
...
Leader:
  ...
  3. Dev Packboy Builder instance polls queue, finds Task-3 matching its role
...
Started via `pcb start --persona dev --team pcb-team`.
Continuously polls the queue for coding tasks:

1. Claims Tasks from queue with role `dev`
2. Reads Packboy Builder's design docs and existing code
...
Started via `pcb start --persona qe --team pcb-team`.
...
Started via `pcb start --persona release --team pcb-team`:
...
at `pcb-team/state/queue.jsonl` (JSONL append-only log).
...
```markdown
You are the Dev agent for the Packboy Builder project.
Your job is to claim coding tasks from the shared queue and implement them.
You are an expert Go developer familiar with Packboy Builder's five-layer package architecture.
```
...
```json
{
  "description": "Packboy Builder project Dev Persona — implements code and submits PRs",
...
```markdown
You are the Leader Agent for the Packboy Builder project — the single entry point
...
```json
{
  "description": "Packboy Builder project Leader Persona — single admin entry point",
...
Leader reads Packboy Builder's `docs/design/`, analyzes existing code, generates a
...
```
Dev Packboy Builder instance polls queue:
...
QE Packboy Builder instance polls queue:
...
Release Packboy Builder instance polls queue:
...
Release Packboy Builder instance:
...
`skills/`, and `settings.json`. Missing parts fall back to Packboy Builder's built-in defaults.

### 2. Personas stored in a separate `pcb-team` repo

Persona definitions live in a dedicated repo, separate from the Packboy Builder source:
- Independent version history for Persona configs
- Different access permissions for the pcb-team repo
...
### 4. Each Persona is an independent Packboy Builder instance

Instead of nested sub-agent calls via the Agent tool, each Persona runs
as an independent Packboy Builder process (`pcb start --persona <name> --team <team>`):
...
Task complete → Persona writes retrospective → identifies improvements
  ...
  → Leader approves
  → Update Persona config in pcb-team repo (via PR)
...
| `pcb start --persona` | Startup-time persona selection (no mid-session switch needed) |
...
### Phase 1 — Create `pcb-team` repo

- Create `pcb-team` repo under `genai-io` Org
- Write four Persona directories (leader/dev/qe/release)
- Each with `system/{identity,behavior,rules}.md` + `skills/` + `settings.json`

### Phase 2 — `pcb start --persona` feature

- Packboy Builder CLI adds `--persona` and `--team` flags
...
- Progress dashboard (CLI: `pcb team status`)
...
2. **Reference design**: Extract patterns (agent separation, adversarial verification,
   multi-stage review) and rewrite for the Packboy Builder project
3. **Priority**: Community skills (marketplace scope) rank below project-custom skills;
   Leader must approve before enabling

## Additional Decisions

1. **Parallel Dev instances**: Each Dev Packboy Builder instance
...
5. **Future team repos**: Beyond `pcb-team`, what infrastructure
