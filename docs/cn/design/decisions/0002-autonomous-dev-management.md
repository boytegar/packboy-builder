...
当前 Packboy Builder 项目（`genai-io/pcb`）的开发依赖人来驱动 Agent：
人在每个会话中手动控制 Agent，不能自动管理整个项目的 issues、feature 和 bug。

目标是把 Packboy Builder 项目变成一个**由 AI 自主管理的项目**：
...
`pcb-team`，专门存放 Packboy Builder 开发团队的 Persona。
...
```
genai-io（Org，已存在）
├── pcb              ← Packboy Builder 源码仓库（已存在，被管理的目标项目）
└── pcb-team         ← 新建的仓库（Packboy Builder 开发团队 Persona）
    ├── leader/
    │   ├── system/
    │   │   ├── identity.md
    │   │   ├── behavior.md
...
- **`pcb-team`**：新建仓库，包含 Packboy Builder 项目的开发团队。
  Persona 直接放在仓库根目录下。如果其他项目需要类似的自主管理，
  可以创建独立的团队仓库（如 `devops-team`）。
- **团队（Team）**：`pcb-team` 仓库中的一组 Persona，
  为特定目标协同工作 —— 管理 Packboy Builder 项目的 issue、feature、bug 和发布。
...
每个 Persona 以**独立的 Packboy Builder 实例**运行：
...
`--persona` 参数告诉 Packboy Builder 在启动时加载哪个 Persona 目录的配置
（system/ + skills/ + settings.json），不需要在运行中通过 `/persona` 命令切换。

多个 Persona 实例可以同时运行在不同的终端、容器或机器上。
它们通过共享工作队列（`pcb-team/state/queue.jsonl`）协调工作。
...
Leader 通过 `pcb start --persona leader --team pcb-team` 启动，
是管理员唯一的交互界面。管理员告诉 Leader 要做什么，
Leader 负责：

1. **理解需求**：新功能？Bug 修复？重构？
2. **分析 Packboy Builder 项目**：读取 Packboy Builder 源码仓库中的设计文档和现有代码
...
由对应 Persona 的 Packboy Builder 实例自动领取执行。
...
Leader:
  ...
  3. Dev Packboy Builder 实例轮询队列，发现 Task-3 匹配自己的角色
...
通过 `pcb start --persona dev --team pcb-team` 启动，
持续轮询队列中的编码任务：

1. 从队列认领 role 为 dev 的 Task
2. 读取 Packboy Builder 项目的设计文档和现有代码
3. 遵循分层架构规范写代码
4. 写测试
...
通过 `pcb start --persona qe --team pcb-team` 启动，
持续轮询队列中的验证任务：
...
通过 `pcb start --persona release --team pcb-team` 启动：
...
`pcb-team/state/queue.jsonl`（JSONL 追加日志）。
...
```markdown
你是 Packboy Builder 项目的开发 Agent。
你的职责是从共享队列领取编码任务并完成实现。
你擅长 Go 开发，熟悉 Packboy Builder 的五层包架构。
```
...
```json
{
  "description": "Packboy Builder 项目开发 Persona，负责编码和 PR 提交",
...
```markdown
你是 Packboy Builder 项目的 Leader Agent，是管理员唯一的交互入口。
...
```json
{
  "description": "Packboy Builder 项目 Leader Persona，管理员的唯一入口",
...
Leader 读取 Packboy Builder 项目的 `docs/design/`，分析已有代码，
生成 mermaid 时序图：
...
Release Packboy Builder 实例轮询队列：
...
`skills/`、`settings.json`。缺失部分自动回退到 Packboy Builder 内置默认值。

### 2. Persona 存于独立的 `pcb-team` 仓库

Persona 定义放在独立仓库 `pcb-team`，与 Packboy Builder 源码仓库分离：
- Persona 配置独立版本管理
- 可以对 pcb-team 仓库设置不同的访问权限
...
### 4. 每个 Persona 是独立的 Packboy Builder 实例

不是通过 Agent 工具嵌套调用子 Agent，而是每个 Persona 启动一个独立的
Packboy Builder 进程（`pcb start --persona <name> --team <team>`）：
...
Task 完成 → Persona 写复盘记录 → 发现可改进项
  ...
  → Leader 审批
  → 更新 pcb-team 仓库中的 Persona 配置（通过 PR）
  → 下次启动时自动加载新配置
```
...
| `pcb start --persona` | 启动时指定 persona，避免启动后热切换 |
...
2. **参考设计**：提取其 Agent 分离、对抗式验证、多阶段审查等模式，重写为适合 Packboy Builder 项目的版本
3. **优先级**：社区技能（marketplace scope）优先级低于项目自定义技能，需要 Leader 审批后启用

## 已决策补充

1. **多 Dev 并行**：每个 Dev Packboy Builder 实例使用独立的 git worktree
