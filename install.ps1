# pcb installer for Windows (PowerShell 5.1+)
#
...
$Binary = 'pcb'
$InstallDir = Join-Path $env:LOCALAPPDATA 'pcb\bin'
...
function Get-Usage {
    ...
    Write-Host ""
    Write-Host "Commands:"
    Write-Host "  install    Install pcb (default)"
    Write-Host "  upgrade    Upgrade to latest version"
    Write-Host "  uninstall  Remove pcb and config"
}
...
function Get-DownloadUrl($version, $arch) {
    return "https://github.com/$Repo/releases/download/v${version}/packboy-builder_windows_${arch}.zip"
}
...
function Invoke-Install {
    ...
    if ($current) {
        if ($current -eq $version) {
            Info "[OK] pcb v$version is already installed"
            return
        }
        Info "Upgrading pcb from v$current to v$version..."
    } else {
        Info "Installing pcb v$version for windows/$arch..."
    }

    $url = Get-DownloadUrl $version $arch
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("pcb-" + [System.Guid]::NewGuid().ToString())
    New-Item -ItemType Directory -Path $tmp -Force | Out-Null
    try {
        $zip = Join-Path $tmp 'pcb.zip'
        ...
        Info "[OK] pcb v$version installed to $ExePath"
    } finally {
...
function Invoke-Uninstall {
    ...
    $cfg = Join-Path $env:USERPROFILE '.pcb'
    if (Test-Path $cfg) {
