...
import (
	...
	"github.com/boytegar/packboy-builder/internal/mcp/transport"
)
...
func requireNpx(t *testing.T) {
	...
	if os.Getenv("PCB_RUN_REAL_MCP") != "1" {
		t.Skip("set PCB_RUN_REAL_MCP=1 to run real MCP server tests")
	}
...
func TestRealMCP_Everything(t *testing.T) {
	...
	// --- Call echo tool ---
	result, err := client.CallTool(ctx, "echo", map[string]any{"message": "hello from pcb"})
	if err != nil {
	...
	if !strings.Contains(result.Content[0].Text, "hello from pcb") {
