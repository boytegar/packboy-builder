...
func TestRegistryLoad_AppliesEnabledStateFromSettings(t *testing.T) {
	...
	pluginDir := filepath.Join(tmpHome, ".pcb", "plugins", "test-plugin")
	manifestDir := filepath.Join(pluginDir, ".pcb-plugin")
...
func TestClaudeCodeCompatibility(t *testing.T) {
	...
	sanManifest := filepath.Join(testPluginDir, ".pcb-plugin", "plugin.json")
	...
	if err != nil {
		t.Fatalf("Failed to read Packboy Builder manifest: %v", err)
	}
	if err := json.Unmarshal(sanContent, &sanData); err != nil {
		t.Fatalf("Failed to parse Packboy Builder manifest: %v", err)
	}
	...
	if sanData["name"] != claudeData["name"] {
		t.Errorf("Name mismatch: Packboy Builder=%v, Claude=%v", sanData["name"], claudeData["name"])
	}
	if sanData["version"] != claudeData["version"] {
		t.Errorf("Version mismatch: Packboy Builder=%v, Claude=%v", sanData["version"], claudeData["version"])
	}
}
