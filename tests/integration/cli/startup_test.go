...
import (
	...
	session "github.com/boytegar/packboy-builder/internal/session"
)

// buildBinary compiles the pcb binary into a temp file and returns its path.
// The binary is removed when the test completes.
func buildBinary(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "pcb-test")
	cmd := exec.Command("go", "build", "-o", bin, "./cmd/pcb")
...
// TestVersionCommand verifies that "pcb version" prints version and build
...
func TestVersionCommand(t *testing.T) {
	...
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("pcb version exited with error: %v", err)
	}
	...
	// First line: pcb version <ver>
	first := strings.TrimSpace(lines[0])
	if !strings.HasPrefix(first, "pcb version ") {
		t.Errorf("expected first line to start with 'pcb version ', got: %q", first)
	}
	ver := strings.TrimPrefix(first, "pcb version ")
	if ver == "" {
...
// TestVersionCommandJSON verifies that "pcb version --json" outputs valid JSON
...
func TestVersionCommandJSON(t *testing.T) {
	...
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("pcb version --json exited with error: %v", err)
	}
...
// TestHelpCommand verifies that "pcb help" exits cleanly and emits usage text
...
func TestHelpCommand(t *testing.T) {
	...
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("pcb help exited with error: %v", err)
	}

	output := string(out)
	for _, expected := range []string{"-p", "--continue", "--resume", "pcb -r <session-id>", "--plugin-dir", "--persona", "version"} {
...
func TestNonInteractivePrintMode(t *testing.T) {
	...
	// ~/.pcb/providers.json is picked up, and unset all API key env vars so
