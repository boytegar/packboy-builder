...
var pluginCmd = &cobra.Command{
	...
	Long: `Manage plugins for extending Packboy Builder with skills, agents, hooks, and MCP servers.
...
Configuration files:
  ~/.pcb/plugins/                  User-level plugins
  ./.pcb/plugins/                  Project-level plugins
  ./.pcb/plugins-local/            Local plugins (git-ignored)
  ~/.pcb/settings.json             Enabled plugins (user)
  ./.pcb/settings.json             Enabled plugins (project)`,
}
...
var pluginListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(plugins) == 0 {
			...
			fmt.Println("  pcb plugin install <plugin>@<marketplace>")
			return nil
		}
...
Examples:
  pcb plugin install git@my-plugins
  pcb plugin install deployment-tools@enterprise-plugins --scope project`,
