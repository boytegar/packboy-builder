...
var mcpCmd = &cobra.Command{
	...
	Long: `Manage MCP servers for extending Packboy Builder with external tools.

MCP servers provide additional tools, resources, and prompts that can be used
by the LLM during conversations.

Configuration files are stored at:
  ~/.pcb/mcp.json           User-level (global)
  ./.pcb/mcp.json           Project-level (team shared)
  ./.pcb/mcp.local.json     Local-level (personal, git-ignored)`,
}
...
For STDIO transport (default):
  pcb mcp add <name> -- <command> [args...]

For HTTP transport:
  pcb mcp add --transport http <name> <url>

For SSE transport:
  pcb mcp add --transport sse <name> <url>

Examples:
  pcb mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem .
  pcb mcp add github --transport http https://api.github.com/mcp
  pcb mcp add sentry --transport sse https://mcp.sentry.dev/mcp`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		case mcp.TransportSTDIO:
			...
			if dashIdx == -1 || dashIdx >= len(os.Args)-1 {
				return fmt.Errorf("STDIO transport requires: pcb mcp add <name> -- <command> [args...]")
			}
		...
		case mcp.TransportHTTP, mcp.TransportSSE:
			if len(args) < 2 {
				return fmt.Errorf("%s transport requires a URL: pcb mcp add --transport %s <name> <url>", mcpTransport, mcpTransport)
			}
			config.URL = args[1]
...
Example:
  pcb mcp add-json filesystem '{"command":"npx","args":["-y","@modelcontextprotocol/server-filesystem","."]}'`,
...
var mcpListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(configs) == 0 {
			...
			fmt.Println("\nAdd a server with:")
			fmt.Println("  pcb mcp add <name> -- <command> [args...]")
			fmt.Println("  pcb mcp add --transport http <name> <url>")
			return nil
		}
