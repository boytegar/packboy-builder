...
var (
	githubLatestRelease = "https://github.com/boytegar/packboy-builder/releases/latest"
...
func runSelfUpdate(ctx context.Context) error {
	...
	assetName := fmt.Sprintf("packboy-builder_%s_%s%s", runtime.GOOS, goArch(runtime.GOARCH), archiveExt)
	downloadURL := fmt.Sprintf("https://github.com/boytegar/packboy-builder/releases/download/v%s/%s", latestVersion, assetName)
	...
	tmpDir, err := os.MkdirTemp(filepath.Dir(exe), ".packboy-update-*")
	if err != nil {
	...
	// Determine binary name (pcb.exe on Windows, pcb on other platforms)
	binName := "pcb"
	if runtime.GOOS == "windows" {
		binName = "pcb.exe"
	}
	...
	fmt.Println("Restart pcb to use the new version.")
	return nil
}
...
func fetchLatestRelease(ctx context.Context) (*releaseInfo, error) {
	...
	// Location: https://github.com/boytegar/packboy-builder/releases/tag/v1.21.4
	version := strings.TrimPrefix(location, "https://github.com/boytegar/packboy-builder/releases/tag/v")
	if version == location {
