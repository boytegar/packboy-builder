...
func TestFetchLatestRelease(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Location", "https://github.com/boytegar/packboy-builder/releases/tag/v1.21.0")
...
func TestDownloadWithProgress(t *testing.T) {
	content := []byte("hello pcb binary content")
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	dir := t.TempDir()
	dest := filepath.Join(dir, "pcb.tar.gz")
...
func TestExtractTarGz(t *testing.T) {
	...
	// Build a tar.gz in memory containing a single "pcb" file
	...
	extracted := filepath.Join(destDir, "pcb")
...
func TestExtractTarGz_WindowsBinary(t *testing.T) {
	...
	// Build a tar.gz containing a "pcb.exe" entry (as expected on Windows)
	var buf bytes.Buffer
	gzw := gzip.NewWriter(&buf)
	tw := tar.NewWriter(gzw)
	...
	extracted := filepath.Join(destDir, "pcb.exe")
	data, err := os.ReadFile(extracted)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtractZip(t *testing.T) {
	...
	// Build a zip in memory containing a single "pcb.exe" file
	var buf bytes.Buffer
	zw := zip.NewWriter(&buf)

	content := []byte("windows binary from zip")
	fw, err := zw.Create("pcb.exe")
	if err != nil {
		t.Fatal(err)
	}
	...
	zipPath := filepath.Join(dir, "packboy-builder_windows_amd64.zip")
	...
	extracted := filepath.Join(dir, "pcb.exe")
	data, err := os.ReadFile(extracted)
	if err != nil {
		t.Fatal(err)
	}
